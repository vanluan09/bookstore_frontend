...
const KeepSakes = () => {
  ...
  const items = [
    getItemProduct('ĐỒ LƯU NIỆM', 'Đồ lưu niệm',  [
      getItemProduct('Tràng hạt', 'Tràng hạt'),
      getItemProduct('Sổ tay', 'Sổ tay'),
    ]),
    
  ];

  const typeKeepSakes = ['Đồ lưu niệm', 'Tràng hạt', 'Sổ tay']; 


  const handleOnCLick = ({ key }) => {
    setKeySelected(key)
    navigate(`/${typeKeepSakes[0].normalize('NFD').replace(/[\u0300-\u036f]/g, '')?.replace(/ /g, '_')}/${key.normalize('NFD').replace(/[\u0300-\u036f]/g, '')?.replace(/ /g, '_')}`,{ state: key }, {replace: true})
  }
  ...
  const renderPage = (key) => {
    switch (key) {
      case 'Tràng hạt':
        return (
          <ChildComponent  product={productTypes['Tràng hạt']} />
        )
      case 'Sổ tay':
        return (
          <ChildComponent product={productTypes['Sổ tay']}/>
        )
      
      default:
        return (
          <ChildComponent  product={productTypes['Đồ lưu niệm']} />
        )
    }
  }
